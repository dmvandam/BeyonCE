...
class ShallotGrid:
    ...
    def _load_grid_attributes(self, directory: str) -> None:
        ...
        self.inclination = GridProperty.load(directory, 
            GridPropertyName.INCLINATION, GridPropertyUnit.DEGREE)
        self.tilt = GridProperty.load(directory, 
            GridPropertyName.TILT, GridPropertyUnit.DEGREE)
        
        self.fx_map = GridProperty.load(directory, 
            GridPropertyName.FX_MAP, GridPropertyUnit.NONE)
        self.fy_map = GridProperty.load(directory, 
            GridPropertyName.FY_MAP, GridPropertyUnit.NONE)
    ...
    def _determine_disk_parameters(self, 
        ...
        x1, y1 = self._calculate_xy_from_parameteric_angle(fx, fy, theta1, 
            shear_value, circular_radius_value)
        x2, y2 = self._calculate_xy_from_parameteric_angle(fx, fy, theta2, 
            shear_value, circular_radius_value)
        ...
        inclination[nan_mask] = np.arccos(semiminor_axis[nan_mask] / 
            semimajor_axis[nan_mask])
    ...
    def _initialise_grid_properties(self) -> None:
        ...
        self.disk_radius = GridProperty(GridPropertyName.DISK_RADIUS, 
            GridPropertyUnit.ECLIPSE_DURATION, np.copy(empty), 
            self.parameters)
        self.inclination = GridProperty(GridPropertyName.INCLINATION, 
            GridPropertyUnit.DEGREE, np.copy(empty), self.parameters)
        self.tilt = GridProperty(GridPropertyName.TILT, 
            GridPropertyUnit.DEGREE, np.copy(empty), self.parameters)
        self.fx_map = GridProperty(GridPropertyName.FX_MAP, 
    ...
    def _interpolate_scale_values(self,
        ...
        # get valid fx and fy
        fy_valid = np.concatenate((fy_interp_x[:-1], 
            np.array([fy_full[min_arg]]), 
            fy_interp_y[1:]), 0)
    ...
    def _set_grid_property_contrast_parameters(self) -> None:
        ...
        if self.diagnostic_map is not None:
        ...
        for y in range(start_y, num_y):
            ...
            for x in tqdm(range(num_x)):
                ...
                fy, disk_radius = self._extend_grid_vertically(
                    fy, disk_radius, x, y)
                fx_valid, fy_valid = self._interpolate_scale_values(fy, 
                    disk_radius, y)
                
                # determine proper grid parameters and set them
                disk_radius_valid, inclination_valid, tilt_valid = (
                    self._determine_disk_parameters(fx_valid, fy_valid,
                        shear_value, circular_radius_value))
    ...
    def diagnose_fxfy_resolution(self) -> None:
        ...
        self.diagnostic_map = GridProperty(
            GridPropertyName.DIAGNOSTIC_MAP, 
            GridPropertyUnit.ECLIPSE_DURATION, 
            diagnostic_values, 
            self.parameters)
    ...
    def _fill_quadrants(self, 
        ...
        if self.parameters.grid_shape != final_shape:
            raise AttributeError(f"the new parameter shape {str(final_shape)}"
        ...
        extended_property = GridProperty(disk_property.name, 
    ...
    def _determine_orbital_scale(self, 
        ...
        limb_darkening = validate.number(limb_darkening, 'limb_darkening', 
            lower_bound=0., upper_bound=1.)

        # determining scale factor
        limb_darkening_scale = ((6 - 2 * limb_darkening) / 
    ...
    def determine_gradients(self, 
        ...
        '''
        # validate
        eclipse_duration = validate.number(eclipse_duration, 
