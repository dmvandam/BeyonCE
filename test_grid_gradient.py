...
from GridProperty import GridGradient
...
@pytest.fixture
def grid_gradient() -> GridGradient:
...
def test_str(grid_gradient: GridGradient) -> None:
...
def test_repr(grid_gradient: GridGradient) -> None:
...
def test_repr_mask(grid_gradient: GridGradient) -> None:
...
def test_set_mask_valid(grid_gradient: GridGradient) -> None:
...
def test_get_scaled_gradient(grid_gradient: GridGradient) -> None:
...
def test_determine_mask_defaults(grid_gradient: GridGradient) -> None:
    measured_gradient = 0.5
    orbital_scale = 1.5
    grid_gradient.determine_mask(measured_gradient, orbital_scale)
    scaled_gradient = grid_gradient.get_scaled_gradient()
...
def test_get_data_unmasked(grid_gradient: GridGradient) -> None:
    mask = np.ones_like(grid_gradient.data).astype(bool)
    grid_gradient.set_mask(mask)
...
def test_get_data_masked(grid_gradient: GridGradient) -> None:
...
def test_get_data_masked_no_mask(grid_gradient: GridGradient) -> None:
    data = grid_gradient.get_data(masked=True)
    expected_data = grid_gradient.data
    assert np.all(data == expected_data)


def test_set_contrast_parameters_none(grid_gradient: GridGradient) -> None:
...
def test_set_contrast_parameters_all_nan(grid_gradient: GridGradient) -> None:
...
def test_set_contrast_parameters(grid_gradient: GridGradient) -> None:
...
def test_get_scaled_gradient_none_set(grid_gradient: GridGradient) -> None:
    scaled_gradient = grid_gradient.get_scaled_gradient()
...
def test_sorting(grid_gradient: GridGradient):
...
def test_save_no_mask(grid_gradient: GridGradient) -> None:
    directory = "test_data"
    if not os.path.exists(directory):
        os.mkdir(directory)

    grid_gradient.save_gradient(directory)
    
    data = np.load(f"{directory}/gradient_{grid_gradient.position:.4f}/"
...
def test_save_with_mask(grid_gradient: GridGradient) -> None:
    directory = "test_data"
    if not os.path.exists(directory):
        os.mkdir(directory)
    ...
    data = np.load(f"{directory}/gradient_{grid_gradient.position:.4f}/"
        f"{grid_gradient.name.name}_{grid_gradient.unit.name}.npy")
    mask_load = np.load(f"{directory}/gradient_{grid_gradient.position:.4f}/"
...
def test_load_invalid() -> None:
    with pytest.raises(LoadError) as ERROR:
        GridGradient.load("invalid")

    message = "failed to load Gradient [None] from invalid"
    assert str(ERROR.value) == message


def test_load_valid_no_mask(grid_gradient: GridGradient) -> None:
    directory = "test_data"
    if not os.path.exists(directory):
        os.mkdir(directory)
    ...
    grid_gradient_load = GridGradient.load(f"{directory}/gradient_"
...
def test_load_valid_with_mask(grid_gradient: GridGradient) -> None:
    directory = "test_data"
    if not os.path.exists(directory):
        os.mkdir(directory)
